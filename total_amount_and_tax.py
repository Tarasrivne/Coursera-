...
def calculate_subtotal(order):
    ...
    sum_of_ord = 0
    for elem in order:
        sum_of_ord += elem['price']
    # for key, val in order.items():
    #     sum_of_ord += val['price']
    return round(sum_of_ord, 2)
...
def calculate_tax(subtotal):
    ...
    ### WRITE SOLUTION HERE
    tax_rate = 15
    tax_rate_subtotal = (subtotal * tax_rate) / 100
    return round(tax_rate_subtotal, 2)
    # tax_of_order = calculate_tax(subtotal)
    # print(tax_of_order)
...
def summarize_order(order):
    ...
    return items_ordered, total_with_tax

    # subtotal = calculate_subtotal(menu)
    # tax_of_order = calculate_tax(subtotal)
    # order_summary = summarize_order(menu)
...
def main():
    ...
    items, total_with_tax = summarize_order(order)
    total = items, round(total_with_tax, 2)
    print(total)
